...
fn test_cli_args_config_path() {
    ...
    assert_eq!(args.unwrap().config_path, Some("./my-config.yaml".to_string()));
}
...
fn test_cli_args_page_404_path() {
    ...
    assert_eq!(args.unwrap().page_404_path, Some("./custom-404.html".to_string()));
}
...
fn test_cli_args_debug_output() {
    ...
    assert!(debug_str.contains("127.0.0.1"), "Debug output should contain IP");
}
...
fn test_cli_args_no_config_with_config_path() {
    ...
    assert_eq!(parsed.config_path, Some("./ignored.yaml".to_string()));
}
